 ...
 */
void sendBeaconMessage(int fd, struct sockaddr_in* client, int len, int broadcast) {
    char msg[512];
    int msg_len;
...
static int rcvBeaconMessage(char* beacon) {
    ...
    for (b = topBeacon; b != NULL; b = b->next) {
        if (strcasecmp(machine, b->machine) == 0 || strcasecmp(identity, b->identity) == 0) {
            break;
        }
    }
    if (b == NULL) {
    ...
    if (lastSummary + 1800 < current) { /* Give a summary of received server beacons every half hour or so */
        len = 0;
        for (b = topBeacon; b != NULL; b = b->next) {
...
void ProcessTiVoBeacon(struct event* ev) {
    ...
    for (n = 0; n < n_lan_addr; n++) {
        if ((sendername.sin_addr.s_addr & lan_addr[n].mask.s_addr) == (lan_addr[n].addr.s_addr & lan_addr[n].mask.s_addr)) {
            break;
        }
    }
    if (n == n_lan_addr) {
