...
static int callback(void* args, int argc, char** argv, char** azColName) {
    ...
    char *id = argv[0], *class = argv[1], *detailID = argv[2], *size = argv[3], *title = argv[4], *duration = argv[5], *bitrate = argv[6], *sampleFrequency = argv[7],
         *artist = argv[8], *album = argv[9], *genre = argv[10], *comment = argv[11], *date = argv[12], *resolution = argv[13], *mime = argv[14];
    ...
    if (strncmp(class, "item", 4) == 0) {
        ...
        if (duration) {
            strcatf(str,
                    "<Duration>%d</Duration>",
                    atoi(strrchr(duration, '.') + 1) + (1000 * atoi(strrchr(duration, ':') + 1)) + (60000 * atoi(strrchr(duration, ':') - 2)) + (3600000 * atoi(duration)));
        }
        if (bitrate) {
...
static void SendItemDetails(struct upnphttp* h, int64_t item) {
    ...
    args.requested = 1;
    xasprintf(&sql,
              SELECT_COLUMNS "from OBJECTS o left join DETAILS d on (d.ID = o.DETAIL_ID)"
...
static void SendContainer(struct upnphttp* h,
                          const char* objectID,
                          int itemStart,
                          int itemCount,
                          char* anchorItem,
    ...
    if (sortOrder) {
        if (strcasestr(sortOrder, "Random")) {
            ...
            while (item != NULL) {
                ...
                } else if (strcasecmp(item, "CreationDate") == 0 || strcasecmp(item, "CaptureDate") == 0) {
                    strcat(order, "DATE");
                    strcat(order2, "DATE");
                } else {
    ...
    if (filter) {
        ...
        for (i = 0; item != NULL; i++) {
            ...
            if ((strcasecmp(item, "x-container/folder") == 0) || (strncasecmp(item, "x-tivo-container/", 17) == 0)) {
