...
#define GET_WAV_INT32(p) ((((uint8_t)((p)[3])) << 24) | (((uint8_t)((p)[2])) << 16) | (((uint8_t)((p)[1])) << 8) | (((uint8_t)((p)[0]))))

#define GET_WAV_INT16(p) ((((uint8_t)((p)[1])) << 8) | (((uint8_t)((p)[0]))))

static int _get_wavtags(char* filename, struct song_metadata* psong) {
    int fd;
    uint32_t len;
     ...
     * in them... */
    if (strncmp((char*)hdr + 0, "RIFF", 4) || strncmp((char*)hdr + 8, "WAVE", 4)) {
    ...
    if (((format_data_length != 16) && (format_data_length != 18)) || (compression_code != 1) || (channel_count < 1)) {
        DPRINTF(E_WARN, L_SCANNER, "Invalid wav header in %s\n", filename);
        return -1;
    }
...
static int _get_wavfileinfo(char* filename, struct song_metadata* psong) {
    psong->lossless = 1;
    /* Upon further review, WAV files should be little-endian, and DLNA requires the LPCM profile to be big-endian.
