...
// _aac_findatom:
static long _aac_findatom(FILE* fin, long max_offset, char* which_atom, int* atom_size) {
...
static int _get_aactags(char* file, struct song_metadata* psong) {
    ...
    if (atom_offset != -1) {
        while (current_offset < atom_length) {
            ...
            } else if (!memcmp(current_atom, "aART", 4) || !memcmp(current_atom, "aart", 4)) {
...
// _get_aacfileinfo
int _get_aacfileinfo(char* file, struct song_metadata* psong) {
    FILE* infile;
    long atom_offset;
    int atom_length;
    int sample_size;
    ...
    if (atom_offset != -1) {
        ...
        if (fread((void*)&sample_size, 1, sizeof(int), infile) != sizeof(int) || fread((void*)&samples, 1, sizeof(int), infile) != sizeof(int)) {
            fclose(infile);
            return -1;
        }
