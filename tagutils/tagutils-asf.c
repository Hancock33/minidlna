...
static inline uint32_t le32_to_cpu(uint32_t le32) {
    ...
    return le32;
#else
    uint32_t be32 = ((le32 << 24) & 0xff000000) | ((le32 << 8) & 0x00ff0000) | ((le32 >> 8) & 0x0000ff00) | ((le32 >> 24) & 0x000000ff);
    return be32;
#endif
}
...
#if __BYTE_ORDER == __LITTLE_ENDIAN
    uint32_t be32 = ((cpu32 << 24) & 0xff000000) | ((cpu32 << 8) & 0x00ff0000) | ((cpu32 >> 8) & 0x0000ff00) | ((cpu32 >> 24) & 0x000000ff);
    return be32;
#else
    return cpu32;
#endif
...
static int _asf_read_file_properties(FILE* fp, asf_file_properties_t* p, uint32_t size) {
    int len;
...
static void _pick_dlna_profile(struct song_metadata* psong, uint16_t format) {
    ...
    case WMAPRO:
        ...
        break;
    case WMALSL:
        xasprintf(&(psong->dlna_pn), "WMALSL%s", psong->channels > 2 ? "_MULT5" : "");
    default:
        break;
    }
}

static int _asf_read_audio_stream(FILE* fp, struct song_metadata* psong, int size) {
...
static int _asf_read_media_stream(FILE* fp, struct song_metadata* psong, uint32_t size) {
    ...
    if (IsEqualGUID(&s.MajorType, &ASF_MediaTypeAudio) && IsEqualGUID(&s.FormatType, &ASF_FormatTypeWave) && s.FormatSize >= sizeof(wfx)) {
...
static int _asf_read_stream_object(FILE* fp, struct song_metadata* psong, uint32_t size) {
...
static int _asf_read_extended_stream_object(FILE* fp, struct song_metadata* psong, uint32_t size) {
    int i, len;
    long off;
    asf_object_t tmp;
...
static int _asf_read_header_extension(FILE* fp, struct song_metadata* psong, uint32_t size) {
    off_t pos;
    long off;
...
static int _asf_load_string(FILE* fp, int type, int size, char* buf, int len) {
...
static void* _asf_load_picture(FILE* fp, int size, void* bm, int* bm_size) {
    int i;
    char buf[256];
#if 0
	//
    ...
    if (!strcasecmp(buf, "image/jpeg") || !strcasecmp(buf, "image/jpg") || !strcasecmp(buf, "image/peg")) {
