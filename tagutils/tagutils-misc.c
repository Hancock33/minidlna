...
#ifdef HAVE_ICONV
static iconv_result do_iconv(const char* to_ces, const char* from_ces, ICONV_CONST char* inbuf, size_t inbytesleft, char* outbuf_orig, size_t outbytesleft_orig) {
    size_t rc;
...
#else  // HAVE_ICONV
static iconv_result do_iconv(const char* to_ces, const char* from_ces, char* inbuf, size_t inbytesleft, char* outbuf_orig, size_t outbytesleft_orig) {
    return ICONV_FATAL;
}
#endif  // HAVE_ICONV
...
static void vc_scan(struct song_metadata* psong, const char* comment, const size_t length) {
    char strbuf[1024];

    if (length > (sizeof(strbuf) - 1)) {
        if (strncasecmp(comment, "LYRICS=", 7) != 0 && strncasecmp(comment, "coverart=", 9) != 0 && strncasecmp(comment, "METADATA_BLOCK_PICTURE=", 23) != 0) {
    ...
    } else if (!strncasecmp(strbuf, "DATE=", 5) || (!strncasecmp(strbuf, "YEAR=", 5) && psong->year == 0)) {
        if (length >= (5 + 10) && isdigit(strbuf[5 + 0]) && isdigit(strbuf[5 + 1]) && ispunct(strbuf[5 + 2]) && isdigit(strbuf[5 + 3]) && isdigit(strbuf[5 + 4])
            && ispunct(strbuf[5 + 5]) && isdigit(strbuf[5 + 6]) && isdigit(strbuf[5 + 7]) && isdigit(strbuf[5 + 8]) && isdigit(strbuf[5 + 9])) {
            // nn-nn-yyyy
            strbuf[5 + 10] = '\0';
