...
static int _get_flctags(char* filename, struct song_metadata* psong) {
    ...
    if (!FLAC__metadata_simple_iterator_init(iterator, filename, true, true)) {
        DPRINTF(E_ERROR, L_SCANNER, "Cannot extract tag from %s [%s]\n", filename, FLAC__Metadata_SimpleIteratorStatusString[FLAC__metadata_simple_iterator_status(iterator)]);
        goto _exit;
    }
    ...
    do {
        ...
        case FLAC__METADATA_TYPE_STREAMINFO:
            ...
            sec = (unsigned int)(block->data.stream_info.total_samples / block->data.stream_info.sample_rate);
            ms = (unsigned int)(((block->data.stream_info.total_samples % block->data.stream_info.sample_rate) * 1000) / block->data.stream_info.sample_rate);
            if ((sec == 0) && (ms == 0)) {
        ...
        case FLAC__METADATA_TYPE_VORBIS_COMMENT:
            for (i = 0; i < block->data.vorbis_comment.num_comments; i++) {
                vc_scan(psong, (char*)block->data.vorbis_comment.comments[i].entry, block->data.vorbis_comment.comments[i].length);
            }
            break;
...
static int _get_flcfileinfo(char* filename, struct song_metadata* psong) {
    psong->lossless = 1;
    psong->vbr_scale = 1;
