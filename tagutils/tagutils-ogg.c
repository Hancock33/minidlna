...
static void _ogg_vorbis_process(ogg_stream_processor* stream, ogg_page* page, struct song_metadata* psong) {
    ogg_packet packet;
...
static void _ogg_vorbis_end(ogg_stream_processor* stream, struct song_metadata* psong) {
    ogg_misc_vorbis_info* inf = stream->data;
    double bitrate, time;
...
static void _ogg_process_null(ogg_stream_processor* stream, ogg_page* page, struct song_metadata* psong) {
    // invalid stream
}

static void _ogg_process_other(ogg_stream_processor* stream, ogg_page* page, struct song_metadata* psong) {
    ogg_stream_pagein(&stream->os, page);
}
...
static void _ogg_other_start(ogg_stream_processor* stream, char* type) {
    if (type) {
        stream->type = type;
    } else {
        stream->type = "unknown";
...
static ogg_stream_processor* _ogg_find_stream_processor(ogg_stream_set* set, ogg_page* page) {
...
static int _ogg_get_next_page(FILE* f, ogg_sync_state* sync, ogg_page* page, ogg_int64_t* written) {
    int ret;
    char* buffer;
    int bytes;
