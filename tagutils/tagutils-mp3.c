...
static int _get_mp3tags(char* file, struct song_metadata* psong) {
    ...
    while ((pid3frame = id3_tag_findframe(pid3tag, "", index))) {
        ...
        if (!strcmp(pid3frame->id, "YTCP")) /* for id3v2.2 */
        {
            ...
            if ((strcmp((char*)id3_field_getlatin1(&pid3frame->fields[1]), "image/jpeg") == 0) || (strcmp((char*)id3_field_getlatin1(&pid3frame->fields[1]), "image/jpg") == 0)
        ...
        if (((pid3frame->id[0] == 'T') || (strcmp(pid3frame->id, "COMM") == 0)) && (id3_field_getnstrings(&pid3frame->fields[1]))) {
            have_text = 1;
        }
...
//   do brute scan
static void __attribute__((unused)) _mp3_get_frame_count(FILE* infile, struct mp3_frameinfo* pfi) {
    int pos;
    int frames = 0;
...
static int _get_mp3fileinfo(char* file, struct song_metadata* psong) {
    ...
    if (strncmp((char*)pid3->id, "ID3", 3) == 0) {
        ...
        /* found an ID3 header... */
        size = (pid3->size[0] << 21 | pid3->size[1] << 14 | pid3->size[2] << 7 | pid3->size[3]);
        fp_size = size + sizeof(struct id3header);
        first_check = 1;

        snprintf(tagversion, sizeof(tagversion), "ID3v2.%d.%d", pid3->version[0], pid3->version[1]);
    ...
    psong->vbr_scale = -1;
    if (!strncasecmp((char*)&buffer[index + fi.xing_offset + 4], "XING", 4)) {
        xing_flags = buffer[index + fi.xing_offset + 4 + 4] << 24 | buffer[index + fi.xing_offset + 4 + 5] << 16 | buffer[index + fi.xing_offset + 4 + 6] << 8
            | buffer[index + fi.xing_offset + 4 + 7];
        psong->vbr_scale = 78;

        if (xing_flags & 0x1) {
            /* Frames field is valid... */
            fi.number_of_frames = buffer[index + fi.xing_offset + 4 + 8] << 24 | buffer[index + fi.xing_offset + 4 + 9] << 16 | buffer[index + fi.xing_offset + 4 + 10] << 8
    ...
    if (!psong->song_length) {
        if (fi.number_of_frames) {
            psong->song_length = (int)((double)(fi.number_of_frames * fi.samples_per_frame * 1000.) / (double)fi.samplerate);
            psong->vbr_scale = 78;
        } else {
            psong->song_length = (int)((double)(file_size - fp_size) * 8. / (double)fi.bitrate);
        }
    }
