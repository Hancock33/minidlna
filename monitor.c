...
int monitor_remove_file(const char* path) {
    ...
    if (playlist) {
        ...
        sql_exec(db,
                 ...
                 MUSIC_PLIST_ID,
                 detailID);
        sql_exec(db, "DELETE from OBJECTS where OBJECT_ID = '%s$%llX' or PARENT_ID = '%s$%llX'", MUSIC_PLIST_ID, detailID, MUSIC_PLIST_ID, detailID);
    } else {
...
int monitor_insert_file(const char* name, const char* path) {
    ...
    len = strlen(path) + 1;
    if (!(path_buf = malloc(len)) || !(last_dir = malloc(len)) || !(base_name = malloc(len))) {
        return -1;
    }
    base_copy = base_name;
    while (depth) {
